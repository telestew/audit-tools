...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
    if (changeInfo.status === 'complete' && tab.url?.includes('outlier.ai')) {
        const { plugins } = await chrome.storage.local.get('plugins');
        if (!plugins) return;
        for (const plugin of plugins) {
            if (plugin.enabled && plugin.contentScripts) {
                for (const cs of plugin.contentScripts) {
                    if (cs.matches.some(m => new RegExp(m.replace(/\\./g, '\\\\.').replace(/\\*/g, '.*')).test(tab.url))) {
